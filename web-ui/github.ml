...
let list_orgs ci =
  ...
  let module OrgId = Ocaml_ci_api.Raw.Reader.OrgId in
  let orgs_f = List.filter (fun x -> OrgId.git_forge_get x == Ocaml_ci_api.Raw.Reader.GitForge.Github) orgs in
  let body = Template.instance Tyxml.Html.[
      breadcrumbs [] "github";
...
let handle ~backend ~meth path =
  ...
  match meth, path with
  | `GET, [] -> list_orgs ci
  | `GET, [owner] -> Capability.with_ref (Client.CI.org ci owner Client.GitHub) @@ list_repos ~owner
  | meth, (owner :: name :: path) ->
    Capability.with_ref (Client.CI.org ci owner Client.GitHub) @@ fun org ->
